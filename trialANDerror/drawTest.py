...
fileway = os.path.dirname(os.path.realpath(__file__))  ##현재디렉토리경로
...
class CView(QGraphicsView):
    ...
    def __init__(self, parent):
        ...
        pixmap = QPixmap()
        pixmap.load("C:/cakeImage/cakebase.png")
        ##pixmap.load(fileway+"/cakebase2.png")
    ...
    def mousePressEvent(self, e):
        ...
        if e.button() == Qt.RightButton:
            ...
            self.end = e.pos()
            pix = QPixmap()
            pix.load(fileway+"/whip2.png")
            scene2 = QGraphicsScene()
            scene2.addPixmap(pix)
            self.setScene(scene2)
            
            self.scene = QGraphicsScene(self.start.x, self.start.y, 50, 50, scene2)
